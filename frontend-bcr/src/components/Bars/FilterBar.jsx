...
import companyIcon from "../../assets/icons/filterbar/company3.png";
import downloadIcon from "../../assets/icons/filterbar/download.svg";
import calendarIcon from "../../assets/icons/filterbar/calendar.svg";
...
const FilterBar = ({ module = "indicadores" }) => {
  ...
  useEffect(() => {
    getInicialData().then((response) => {
      ...
      if ("success" in response && response.success) {
        ...
        setSelectedData({
          ...selectedData,
          tipo_indicador: response.payload.tipo_indicador[1],
      ...
      <section className="cursor-pointer relative group/menu_filtros py-1 w-[200px] lg:w-full flex flex-row gap-3 justify-center items-center text-slate-200/90 font-medium">
        ...
        <div className="z-[700] px-2 lg:flex duration-300 lg:top-0 lg:left-0 md:top-[4px] md:left-[calc(50%+70px)] top-[30px] m-auto md:md-none hidden lg:relative absolute group-hover/menu_filtros:flex hover:flex rounded-b-md outline-none cursor-pointer gap-2 text-slate-200 font-medium">
          <ul className="bg-cyan-300 py-1 lg:py-0 shadow-[0px_0px_10px_1px_#00000088] lg:shadow-none px-1 rounded-md lg:bg-transparent w-full flex flex-col lg:flex-row gap-1 lg:gap-3 justify-center items-center text-slate-200/90 font-medium">
            <li
              className="hover:shadow-[0px_0px_2px_2px_#0cd4d4] w-[240px] h-6 relative group/item px-1 flex flex-row justify-center items-center rounded-md gap-2 text-slate-800/90 font-extrabold border-[1px] shadow-[4px_6px_20px_-2px_#00000066]  border-[#00000015] bg-gradient-to-r from-sky-100/90 via-sky-100 to-sky-100/90"

              // className="hover:shadow-[0px_0px_2px_2px_#0cd4d4] w-[240px] h-6 relative group/item px-2 flex flex-row justify-center items-center rounded-md gap-2 text-slate-800/90 font-extrabold border-[1px] shadow-[4px_6px_20px_-2px_#00000066]  border-[#00000015] bg-gradient-to-r from-sky-100/90 via-sky-100 to-sky-100/90"
            >
              <section className="w-full h-full flex flex-row justify-between items-center">
                <div className="capitalize flex flex-row gap-1">
                  <div>Agencia</div>
                  <div className="text-cyan-500">
                    {selectedData.agencia.toLowerCase()}
                  </div>
                </div>
                <img
              ...
              <ul className="z-[500] h-[300px] px-1 py-1 w-full flex-col overflow-y-auto duration-300 top-[22px] hidden absolute group-hover/item:flex hover:flex shadow-[0px_0px_15px_0px_#ffffffdd] rounded-b-md outline-none cursor-pointer bg-slate-300 gap-2 text-slate-200 font-medium">
            ...
            >
              <section className="w-full h-full flex flex-row justify-between items-center">
                <div className="capitalize flex flex-row gap-1">
                  <div>Indicador</div>
                  <div className="text-cyan-500">
                    {selectedData.tipo_indicador.toLowerCase()}
                  </div>
                </div>
                <img
                  className="h-4 w-4 filter contrast-[30%] outline-none group-hover/item:contrast-100 group-hover/item:rotate-180 duration-300 ease-in-out"
              ...
              <ul className="z-[500] px-1 py-1 w-full flex-col  duration-300 top-[22px] hidden absolute group-hover/item:flex hover:flex shadow-[0px_0px_15px_0px_#ffffffdd] rounded-b-md outline-none cursor-pointer bg-slate-300 gap-2 text-slate-200 font-medium">
            ...
            >
              <section className="w-full h-full flex flex-row justify-between items-center">
                <div className="capitalize flex flex-row gap-1">
                  <div>Periodo</div>
                  <div className="text-cyan-500">
                    {selectedData.fecha.toLowerCase()}
                  </div>
                </div>
                <img
                  className="h-4 w-4 filter contrast-[30%] outline-none group-hover/item:contrast-100 group-hover/item:rotate-180 duration-300 ease-in-out"
            ...
            <li className="w-[180px] xl:w-[140px] cursor-pointer gap-2 flex justify-center items-center">
