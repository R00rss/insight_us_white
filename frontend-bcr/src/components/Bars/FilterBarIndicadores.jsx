...
function mapeoIndicadores(str) {
  if (str.toLowerCase() === "experiencia".toLowerCase()) return "lealtad";
  if (str.toLowerCase() === "promesa de servicio".toLowerCase()) return "atributos de servicio";
  return str;
}
function mapeoSubindicadores(str) {
  if (str) {
    console.log(str)
    if (str.toLowerCase() === "esfuerzo".toLowerCase()) return "(CES)" + str;
    if (str.toLowerCase() === "satifacción general".toLowerCase()) return "(INS)" + str;
    if (str.toLowerCase() === "recomendación".toLowerCase()) return "(NPS)" + str;
  }
  return str;
}
...
const default_options_filter_bar = {
  ...
  subindicador: {
    ...
    options: { all: [], lealtad: [], 'atributos de servicio': [] },
  },
  periodo: {
      ...
      if ("success" in response && response.success) {
        const options_filter_bar_temp = {
          ...
          indicador: {
            ...
            options: {
              all: response.payload.indicadores.map(indicador => {
                return {
                  display_name: mapeoIndicadores(indicador),
                  name: indicador,
                  value: false
                }
              })
          ...
          subindicador: {
            ...
            name: "subindicador",
            options: {
              'lealtad': response.payload.subindicadores[response.payload.indicadores[0]].map(subindicador => {
                return {
                  display_name: mapeoSubindicadores(subindicador),
                  name: subindicador,
                  value: false
                }
              }),
              'atributos de servicio': response.payload.subindicadores[response.payload.indicadores[1]].map(subindicador => {
                return {
                  display_name: mapeoSubindicadores(subindicador),
                  name: subindicador,
                  value: false
                ...
                >
                  ...
                  >
                    ...
                    {option_filter_bar_key === "subindicador" ? (<>
                      {options_filter_bar[option_filter_bar_key].options[`${options_filter_bar.indicador.options.all[0]?.value ? 'leatad' : 'atributos de servicio'}`].map((option, j) => (
                        <li
                          key={j}
                          className="capitalize bg-[#0cd4d4bb] rounded-md px-1 hover:bg-[#0cd4d477]"
                            ...
                            (prev) => ({
                              ...
                              [option_filter_bar_key]: {
                                ...
                                options: {
                                  ...
                                  [`${options_filter_bar.indicador.options.all[0]?.value ? 'leatad' : 'atributos de servicio'}`]: prev[option_filter_bar_key].options[`${options_filter_bar.indicador.options.all[0]?.value ? 'experiencia' : 'promesa de servicio'}`].map((item, i) => {
                                    if (i === j) {
                                      return {
                                        ...item,
                                        value: !item.value,
                        ...
                        >
                          <div
                            ...
                            <div>
                              {minimizeStr(option.display_name.toLowerCase(), 15)}
                            </div>
